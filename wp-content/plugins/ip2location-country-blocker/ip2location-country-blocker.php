...
class IP2LocationCountryBlocker
{
	...
	public function frontend_page()
	{
		...
		if (isset($_POST['submit'])) {
			...
			if (!empty($frontend_auto_block_threshold) && !preg_match('/^[0-9]+$/', $frontend_auto_block_threshold)) {
				$frontend_status = '
				<div class="error">
					<p><strong>ERROR</strong>: Auto block threshold has to be a number.</p>
				</div>';
					...
					<tr>
						...
						<td>
							...
							<div style="margin-top:10px">
								<select name="frontend_block_proxy_type[]" id="frontend_block_proxy_type" data-placeholder="Choose Proxy Type..." multiple="true" class="chosen input-field">';
					...
					<tr>
						...
						<td>
							<fieldset>
								...
								<p class="description">Use asterisk (*) for wildcard matching. E.g.: 8.8.8.* will match IP from 8.8.8.0 to 8.8.8.255. CIDR format also supported.</p>
							</fieldset>
						</td>
					</tr>
					<tr>
						<th scope="row">
							<label>Automatic Blacklist</label>
						</th>
						<td>
							...
							<p class="description">Automatically add client IP into blacklist if client keep hitting front pages more than this threshold within 24 hours.</p>
						</td>
					</tr>
					<tr>
						<th scope="row">
		...
		echo '
					...
					<tr>
						...
						<td>
							...
							<p class="description">Automatically add client IP into blacklist if client keep hitting admin area more than this threshold within 24 hours.</p>
						</td>
					</tr>
					<tr>
						<th scope="row">
	...
	public function statistics_page()
	{
		...
		<div class="wrap">
			...
			' . ((get_option('ip2location_country_blocker_log_enabled')) ? '' : '<div class="update-message notice inline notice-warning notice-alt">Visitor log is disabled. Please enable it in <a href="' . admin_url('admin.php?page=ip2location-country-blocker-settings') . '">Settings</a> page to collect statistics data.</div>') . '
					...
					<tr>
						...
						<td>
							<select name="lookup_mode" id="lookup_mode"' . (($disabled) ? ' disabled' : '') . '>
								<option value="bin"' . (($lookup_mode == 'bin') ? ' selected' : '') . '> Local BIN Database</option>
								<option value="ws"' . (($lookup_mode == 'ws') ? ' selected' : '') . '> API Web Service</option>
							<select>
						</td>
					</tr>
					<tr>
							...
							<div id="bin_database"' . (($lookup_mode == 'ws') ? ' style="display:none"' : '') . '>
								<div class="iplcb-panel">
									<table class="form-table">
										...
										<tr>
											...
											<td>
												<label for="download_ipv4_only">
													<input type="checkbox" name="download_ipv4_only" id="download_ipv4_only" value="true"' . (($download_ipv4_only) ? ' checked' : '') . (($disabled) ? ' disabled' : '') . '> Download IPv4 database only
												</label>

												<p class="description">
													Download a smaller database which is faster in lookup speed. Perfect for website with only IPv4 supported.
												</p>
											</td>
										</tr>
										<tr>
											...
											<td>
												...
												' . ((preg_match('/LITE/', get_option('ip2location_country_blocker_database'))) ? '<p class="description">If you are looking for high accuracy result, you should consider using the commercial version of <a href="https://www.ip2location.com/database/db1-ip-country#wordpress-wzdicb" target="_blank">IP2Location BIN database</a></p>' : '') . '
											</td>
										</tr>
										<tr>
											<th scope="row">
										...
										<tr>
											<td></td>
											<td><button id="update_ip2location_database" type="button" class="button button-secondary"' . (($disabled) ? ' disabled' : '') . '>Update Database</button></td>
										</tr>
									</table>
								</div>
							</div>
								...
								<div class="iplcb-panel">
									...
									<tr>
										...
										<td>
											...
											<p class="description">Your IP2Location <a href="https://www.ip2location.com/web-service/ip2location" target="_blank">Web service</a> API key.</p>
										</td>
									</tr>';
					...
					<tr>
						...
						<td>
							<select name="px_lookup_mode" id="px_lookup_mode"' . (($disabled) ? ' disabled' : '') . '>
								<option value=""' . (($px_lookup_mode == '') ? ' selected' : '') . '> Disabled</option>
								<option value="px_bin"' . (($px_lookup_mode == 'px_bin') ? ' selected' : '') . '> Local BIN Database</option>
								<option value="px_ws"' . (($px_lookup_mode == 'px_ws') ? ' selected' : '') . '> API Web Service</option>
							<select>
						</td>
					</tr>
					<tr>
								...
								<div class="iplcb-panel">
									...
									<tr>
										...
										<td>
											...
											<p class="description">Your IP2Proxy <a href="https://www.ip2location.com/web-service/ip2proxy" target="_blank">Web service</a> API key.</p>
										</td>
									</tr>';
		...
		if (!$this->is_setup_completed()) {
			...
			<div id="modal-get-started" class="ip2location-modal" style="display:block">
				<div class="ip2location-modal-content" style="width:400px;height:250px">
					...
					<p>
						<strong>' . __('IP2Location Country Blocker', 'ip2location-country-blocker') . '</strong> ' . __('is a plugin to block visitor by their location', 'ip2location-country-blocker') . '.
					</p>
					<p>
	...
	public function check_block()
	{
		...
		if ($this->is_backend_page()) {
			...
			if (get_option('ip2location_country_blocker_backend_auto_block_threshold')) {
				$GLOBALS['wpdb']->query('INSERT INTO ' . $GLOBALS['wpdb']->prefix . 'ip2location_country_blocker_rate_limit_log(ip_address, date_created) VALUES ("' . $this->get_ip() . '", "' . date('Y-m-d H:i:s') . '")');

				$total = $GLOBALS['wpdb']->get_var('SELECT COUNT(*) FROM ' . $GLOBALS['wpdb']->prefix . 'ip2location_country_blocker_rate_limit_log WHERE ip_address = "' . $this->get_ip() . '" AND date_created >= "' . date('Y-m-d H:i:s', strtotime('-24 hour')) . '";');
				...
				$GLOBALS['wpdb']->query('DELETE FROM ' . $GLOBALS['wpdb']->prefix . 'ip2location_country_blocker_rate_limit_log WHERE ip_address = "' . $this->get_ip() . '" AND date_created < "' . date('Y-m-d H:i:s', strtotime('-24 hour')) . '";');
			}
		}
	...
	{
		add_menu_page('Country Blocker', 'Country Blocker', 'manage_options', 'ip2location-country-blocker', [$this, 'frontend_page'], 'dashicons-admin-ip2location', 30);
		add_submenu_page('ip2location-country-blocker', 'Frontend', 'Frontend', 'manage_options', 'ip2location-country-blocker', [$this, 'frontend_page']);
		add_submenu_page('ip2location-country-blocker', 'Backend', 'Backend', 'manage_options', 'ip2location-country-blocker-backend', [$this, 'backend_page']);
		add_submenu_page('ip2location-country-blocker', 'Statistics', 'Statistics', 'manage_options', 'ip2location-country-blocker-statistics', [$this, 'statistics_page']);
		add_submenu_page('ip2location-country-blocker', 'IP Lookup', 'IP Lookup', 'manage_options', 'ip2location-country-blocker-ip-lookup', [$this, 'ip_lookup_page']);
		add_submenu_page('ip2location-country-blocker', 'Settings', 'Settings', 'manage_options', 'ip2location-country-blocker-settings', [$this, 'settings_page']);
	}
	...
	public function show_notice()
	{
		...
		<div class="error">
			<p>
				IP2Location Country Blocker requires the IP2Location BIN database to work. <a href="' . get_admin_url() . 'admin.php?page=ip2location-country-blocker-settings">Setup your database</a> now.
			</p>
		</div>';
	}
	...
	private function create_table()
	{
		...
		$GLOBALS['wpdb']->query('
		CREATE TABLE IF NOT EXISTS ' . $GLOBALS['wpdb']->prefix . 'ip2location_country_blocker_rate_limit_log (
			`log_id` INT(11) NOT NULL AUTO_INCREMENT,
			`ip_address` VARCHAR(39) NOT NULL COLLATE \'utf8_bin\',
			`date_created` DATETIME NOT NULL,
			PRIMARY KEY (`log_id`),
	...
	private function within_cidr($ip, $range)
	{
		if (filter_var($ip, \FILTER_VALIDATE_IP, \FILTER_FLAG_IPV4)) {
